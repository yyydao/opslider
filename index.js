...
var Opslider = function (_config) {
    var util = Opslider.Utils,
        config;
    this.config = util.merge({}, _default, _config || {});
...
Opslider.prototype.init = function () {
    ...
    this.set("panelNode", this.get("pageNode").querySelectorAll("." + this.get("panel")));
    len = this.get("panelNode").length;
    if (!len || len < 2) {
        return
    }
            ...
            if (next) {
                ...
                if (Math.abs(disy) > _this.get("diff")) {
                    ...
                    setTimeout(function () {
                        ...
                        _this.emit('show',next.getAttribute("data-index"));
